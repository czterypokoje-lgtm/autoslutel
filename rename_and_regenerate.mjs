...
files.forEach((file, index) => {
  ...
  // e.g. bmw_1serie_sleutel_bijmaken_amsterdam.png
  let newFilename = `${safeBrand}_${safeModel}_sleutel_bijmaken_${safeCity}.png`;
  
  // Ensure unique filename if somehow duplicate (e.g. index)
  newFilename = newFilename.replace('.png', `_${index}.png`);
  ...
  const templates = [
    `Vandaag op locatie in **${cityObj.c}**: een nieuwe autosleutel geprogrammeerd voor deze **${printBrand} ${printModel}**, in slechts 30 minuten. Geen dealerwachttijd, geen sleepkosten — wij komen naar u toe, in heel ${cityObj.c} en de regio ${cityObj.r}.\n\n1 jaar garantie op elke sleutel. Sleutel kwijt of kapot? Bel ons direct voor een vaste prijs.`,
    
    `**${printBrand}** sleutel kwijt of buitengesloten in **${cityObj.c}**? Wij maken direct op locatie een nieuwe sleutel. Geen sleepkosten, 25 min service. Bel 24/7!\n\nWeer een tevreden klant geholpen in ${cityObj.c}! Heeft u een nieuwe sleutel nodig voor uw ${printBrand} (zoals deze ${printModel} uit ${year})? Bespaar dure sleepkosten naar de dealer. Autosleutel24 is direct ter plaatse en programmeert binnen 25 minuten een compleet nieuwe smart key. Inclusief 1 jaar garantie! Staat u stil? Gebruik de 'Bel Nu' knop voor directe hulp.`,
    
    `Sleutelprobleem met uw **${printBrand}** in **${cityObj.c}**? 🚗 Autosleutel24 is uw mobiele slotenmaker voor auto's. Vandaag hebben we de eigenaar van deze **${printBrand} ${printModel}** direct weer op weg geholpen.\n\n✅ Snel ter plaatse in regio ${cityObj.r}\n✅ Dealer kwaliteit, maar veel sneller\n✅ Vaste prijs vooraf en 1 jaar garantie\n\nHeeft u ook een nieuwe autosleutel nodig? Bel of WhatsApp ons 24/7!`,
    
    `Nieuwe sleutel nodig voor uw **${printBrand}**? 🔑 In **${cityObj.c}** hebben we vandaag succesvol een nieuwe transpondersleutel ingeleerd voor deze **${printBrand} ${printModel}**. Waarom weken wachten bij de dealer als het ook in 30 minuten op locatie kan?\n\nWij zijn 24/7 actief in ${cityObj.c} en omstreken. Bel ons voor een directe prijsopgave en we komen meteen naar u toe!`
  ];
  ...
  output += `- **Post Title (Başlık):** ${printBrand} ${printModel} (${year}) sleutel bijmaken in ${cityObj.c} ✅\n`;
...
console.log('Renamed and regenerated successfully!');
