...
export default function ProductAccordions({ product }: { product: ShopProduct }) {
  // Specifications first: it is the block people come down here for.
  ...
  const titleStyle = { fontSize: '1.25rem', fontWeight: 800, margin: 0, color: '#0f172a' };
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      ...
      <div style={containerStyle}>
        ...
        {openSection === 'specs' && (
          <div style={{ padding: '1.5rem 2rem 2rem', borderTop: '1px solid #f1f5f9' }}>
            {specs.length > 0 ? (
      ...
      {/*
        ...
        testimonials signed "Mark R." and "Jeroen B." — the same on every
        product. Invented reviews are a misleidende handelspraktijk (BW 6:193c)
        and marking them up would risk a manual action from Google.

        Bring this back wired to real, verified-purchase reviews, and only then
        add AggregateRating to the Product schema.
      */}
